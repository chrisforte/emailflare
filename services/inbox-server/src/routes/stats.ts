...
app.get('/', async (c) => {
  ...
  const { domains, templates, apiKeys, emailLogs } = makeDb();

  const [
    totalDomains, verifiedDomains, totalTemplates, totalKeys,
    totalEmails, sentToday, failedToday,
    domainBreakdownResult, dailyResult,
  ] = await Promise.all([
    domains.count(),
    ...
    rawDb.query<{ domain_name: string; verified: number; sent: number; failed: number }>(
      `SELECT d.name AS domain_name, d.verified,
    ...
    rawDb.query<{ date: string; sent: number; failed: number }>(
      `SELECT strftime('%Y-%m-%d', sent_at) AS date,
         SUM(CASE WHEN status = 'sent'   THEN 1 ELSE 0 END) AS sent,
         SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END) AS failed
       FROM email_logs WHERE sent_at >= ?
  ...
  return c.json({
    range, totalDomains, verifiedDomains, totalTemplates, totalKeys,
    totalEmails, sentToday, failedToday, cfDailyLimit: CF_DAILY_LIMIT,
    domainBreakdown: domainBreakdownResult.rows.map(r => ({
      name: r.domain_name, verified: r.verified === 1, sent: r.sent, failed: r.failed,
    })),
