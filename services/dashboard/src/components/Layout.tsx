...
import {
  ...
  MonitorDot, BookOpen, Github, ExternalLink, Inbox, Users, ListOrdered,
  Settings, Mail,
} from 'lucide-react';
...
function buildNavSections(user: User | null) {
  const sections = [
    {
      label: 'Monitor',
      items: [
    ...
    },
    {
      label: 'Inbox',
      items: [
        { to: '/inbox',           label: 'People',    icon: Mail },
        { to: '/inbox/sequences', label: 'Sequences', icon: ListOrdered },
      ],
    },
    {
      label: 'Send',
    ...
    },
    {
      ...
      items: [
        ...
        { to: '/inbox/settings', label: 'Inboxes',  icon: Inbox },
      ],
    },
  ];
...
function AppSidebar({ user, onLogout }: { user: User | null; onLogout: () => void }) {
      ...
      <SidebarFooter className="border-t border-sidebar-border px-3 py-3 gap-0.5">
        ...
        <div className="border-t border-sidebar-border pt-3 flex items-center gap-2.5 px-2.5 py-2 mb-0.5">
          ...
          <div className="leading-none min-w-0 flex-1">
            <p className="text-[12.5px] font-semibold text-foreground truncate">{user?.name ?? 'Loading…'}</p>
            <p className="text-[10.5px] text-muted-foreground mt-0.5 truncate">{user?.email}</p>
          </div>
          {user?.role === 'admin' && (
